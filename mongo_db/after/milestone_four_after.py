...
# PEGD
def _highest_pegd(request, collection, n=5):
    """Find n number of stocks with the highest PEGD ratio."""
    collection = db[collection]
    query_params_dict = dict(request.query)
    print(query_params_dict)
            ...
            [
                {
                    "$project": {
                        ...
                        "Dividend Yield": 1,
                        "g+d": {
                            "$add": ["$EPS growth past 5 years", "$Dividend Yield"]
                        },
                    }
                },
                {
...
@route(
    "/<collection>/api/{version}/highestPegd".format(version=version),
    method="GET"
)
@route(
    "/<collection>/api/{version}/highestPegd/<number>".format(version=version),
    method="GET"
)
def aggregation_pipeline(collection, number=5):
    """API Wrapper for aggregation pipeline."""
    return _highest_pegd(request, collection, n=number)
